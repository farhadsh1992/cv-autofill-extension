...
// text: the cover letter body, blank-line-separated paragraphs (AI output
// already comes this way). A single newline within a paragraph is treated
// as a soft wrap and joined with a space, not a new paragraph.
function generateCoverLetterDocx(text) {
  const blocks = (text || "").trim().split(/\n{2,}/);
  const paragraphs = blocks.map((block, i) =>
    docxParagraph(block.split("\n").join(" "), { spacingAfter: i === blocks.length - 1 ? 0 : 200 })
  );
  if (!paragraphs.length) paragraphs.push(docxParagraph(""));

  const sectPr = `<w:sectPr><w:pgSz w:w="12240" w:h="15840"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr>`;
...
// ---- Conferences / Journals trackers (same table-doc shape as Jobs) ----

// conferences: [{ name, deadline, location, status, link, notes }]
function generateConferencesDocx(conferences) {
  const headers = ["Name", "Deadline", "Location", "Status", "Link", "Notes"];
  // dxa (twentieths of a point); landscape letter minus 1" margins each side = 13680 dxa available.
  const widths = [2400, 1500, 2200, 1800, 2180, 3600];
  ...
  const paragraphs = [
    docxParagraph([{ text: "Conferences", bold: true, size: 32 }], { spacingAfter: 200 }),
    docxTable(headers, rows, widths),
    // OOXML requires a paragraph after a table that's the last block in the body.
    docxParagraph(""),
  ];
...
// journals: [{ name, fullName, link, status, notes, addedAt }]
function generateJournalsDocx(journals) {
  const headers = ["Name", "Full name", "Status", "Link", "Notes", "Date"];
  // dxa (twentieths of a point); landscape letter minus 1" margins each side = 13680 dxa available.
  const widths = [1800, 3200, 1600, 2180, 3400, 1500];
  ...
  const paragraphs = [
    docxParagraph([{ text: "Journals", bold: true, size: 32 }], { spacingAfter: 200 }),
    docxTable(headers, rows, widths),
    // OOXML requires a paragraph after a table that's the last block in the body.
    docxParagraph(""),
  ];
...
if (typeof self !== "undefined") {
  ...
  self.generateCoverLetterDocx = generateCoverLetterDocx;
  self.docxFormatJobDate = docxFormatJobDate;
  self.generateConferencesDocx = generateConferencesDocx;
  self.generateJournalsDocx = generateJournalsDocx;
}
